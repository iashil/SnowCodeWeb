...
export function createLocalAdminSession(rememberMe = false) { const payload = JSON.stringify({ email: LOCAL_ADMIN_EMAIL, expiresAt: Date.now() + (rememberMe ? REMEMBERED_SESSION_TTL_MS : SESSION_TTL_MS) }); return `${Buffer.from(payload).toString("base64url")}.${sign(payload)}`; }
export function getLocalAdminFromRequest(request: Request) { const token = parseCookies(request)[LOCAL_ADMIN_COOKIE]; if (!token) return null; const separator = token.lastIndexOf("."); if (separator < 1) return null; const encodedPayload = token.slice(0, separator); const signature = token.slice(separator + 1); let payload: string; try { payload = Buffer.from(encodedPayload, "base64url").toString("utf8"); } catch { return null; } if (sign(payload) !== signature) return null; try { const parsed = JSON.parse(payload) as { email?: string; expiresAt?: number }; if (parsed.email !== LOCAL_ADMIN_EMAIL || !parsed.expiresAt || parsed.expiresAt < Date.now()) return null; return localAdminUser; } catch { return null; } }
