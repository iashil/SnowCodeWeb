...
export function createLocalAdminSession() { const payload = JSON.stringify({ email: LOCAL_ADMIN_EMAIL, expiresAt: Date.now() + SESSION_TTL_MS }); return `${Buffer.from(payload).toString("base64url")}.${sign(payload)}`; }
