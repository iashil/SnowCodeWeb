...
export const appRouter = router({
  ...
  auth: router({
    ...
    localLogin: publicProcedure.input(z.object({ email: z.string().min(3).max(320), password: z.string().min(1).max(200) })).mutation(async ({ input, ctx }) => { const user = await authenticateLocalAdmin(input.email, input.password); if (!user) throw new TRPCError({ code: "UNAUTHORIZED", message: "Invalid admin credentials" }); ctx.res.cookie(LOCAL_ADMIN_COOKIE, createLocalAdminSession(), { ...getSessionCookieOptions(ctx.req), maxAge: 1000 * 60 * 60 * 24 * 7 }); return { success: true, user } as const; }),
